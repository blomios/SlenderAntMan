    ...
    public class Chunk : MonoBehaviour
    {
...
#if UNITY_EDITOR
            if (chunkLodGroup)
                chunkLodGroup.UpdateStaticEditorFlags();
#endif
        }
        ...
        public void CreateWithoutOperation()
        {
            for (var lodIndex = 0; lodIndex < chunkLodGroup.LODCount; ++lodIndex) {
                ...
                var collisionMesh = lodIndex == digger.ColliderLodIndex || chunkLodGroup.LODCount == 1 ? voxelChunk.BuildVisualMesh(lod) : null;
#else
                var collisionMesh = lodIndex == digger.ColliderLodIndex || chunkLodGroup.LODCount == 1 ? voxelChunk.BuildCollisionMesh(lod) : null;
#endif
                var res = chunkLodGroup.PostBuild(lodIndex, null, collisionMesh, new ChunkTriggerBounds(digger.HeightmapScale, digger.SizeOfMesh));
                if (lodIndex == 0)
                    hasVisualMesh = res;
            }
        }
                           ...
                           float radius, float coneHeight, bool upsideDown, int textureIndex, bool cutDetails)
        {
            ...
            voxelChunk.DoOperation(brush, action, intensity, center, radius, coneHeight, upsideDown, textureIndex, cutDetails);

            for (var lodIndex = 0; lodIndex < chunkLodGroup.LODCount; ++lodIndex) {
                var lod = ChunkLODGroup.IndexToLod(lodIndex);
                var visualMesh = voxelChunk.BuildVisualMesh(lod);
#if UNITY_2019_3_OR_NEWER
                var collisionMesh = lodIndex == digger.ColliderLodIndex || chunkLodGroup.LODCount == 1 ? visualMesh : null;
#else
                var collisionMesh = lodIndex == digger.ColliderLodIndex || chunkLodGroup.LODCount == 1 ? voxelChunk.BuildCollisionMesh(lod) : null;
#endif
